...
CREATE TABLE branch (
    ...
    UNIQUE (address_id), -- We have 2 seperate unqiues as theyre individually unqiue
    UNIQUE (branch_name)
...
SELECT c.first_name, c.last_name, c.customer_id, a.account_number, a.account_type_id
FROM customer c
...
-- Create view to have a temporary table of customer account and transaction data we can use for further queries
CREATE VIEW customer_account_transaction_details 
AS
...
SELECT opening_balance + SUM(IF(isIncoming, amount, 0)) - SUM(IF(isOutgoing, amount, 0)) current_balance,
...
SELECT 
    ...
    FROM transaction WHERE isOutgoing) total_outstanding -- Call the result 'total_outstanding'
FROM account a;